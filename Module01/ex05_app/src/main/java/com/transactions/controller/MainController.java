...
public class MainController {
    ...
    private void buildSidebar() {
        ...
        btnBalance.setOnAction(e  -> { refreshUserRows(); showPanel(viewBalancePanel); });
    ...
    private ToggleButton navButton(String text, ToggleGroup group) {
        ...
        btn.selectedProperty().addListener((obs, wasSelected, isNow) -> {
            if (isNow) btn.getStyleClass().add("active");
    ...
    private Node buildAddUserPanel() {
        ...
        submit.setOnAction(e -> {
            ...
            if (name.isEmpty()) { nameField.getStyleClass().add("error"); toast("Name is required", Toast.Type.ERROR); return; }
            int balance;
            try { balance = Integer.parseInt(balText); } catch (Exception ex) {
                balanceField.getStyleClass().add("error"); toast("Balance must be a number", Toast.Type.ERROR); return;
            }
    ...
    private Node buildViewBalancePanel() {
        ...
        refreshBtn.setOnAction(e -> { refreshUserRows(); toast("User list refreshed", Toast.Type.INFO); });
    ...
    private Node buildTransferPanel() {
        ...
        submit.setOnAction(e -> {
            ...
            try {
                ...
                senderField.clear(); recipientField.clear(); amountField.clear();
    ...
    private Node buildViewTransactionsPanel() {
        ...
        fetchBtn.setOnAction(e -> {
            ...
            try {
                ...
                if (txns.length == 0) { toast("No transactions for this user", Toast.Type.INFO); return; }
    ...
    private Node buildRemoveTransferPanel() {
        ...
        devBadge.setStyle("-fx-background-color: #fef3c7; -fx-text-fill: #92400e; -fx-padding: 6 14; -fx-background-radius: 6; -fx-font-weight: bold; -fx-font-size: 11px;");
        ...
        submit.setOnAction(e -> {
            ...
            try {
                ...
                UUID txnId;
                try { txnId = UUID.fromString(txnIdField.getText().trim()); } catch (Exception ex) {
                    txnIdField.getStyleClass().add("error"); toast("Invalid UUID format", Toast.Type.ERROR); return;
                }
                service.removeTransaction(uid, txnId);
                toast("Transfer removed successfully", Toast.Type.SUCCESS);
                userIdField.clear(); txnIdField.clear();
    ...
    public static class UserRow {
        ...
        public UserRow(int id, String name, String balanceDisplay) {
            this.id = id; this.name = name; this.balanceDisplay = balanceDisplay;
        }
    ...
    public static class TxnRow {
        ...
        public TxnRow(String direction, String peer, String amountDisplay, String uuid) {
            this.direction = direction; this.peer = peer; this.amountDisplay = amountDisplay; this.uuid = uuid;
        }
    ...
    public static class UnpairedRow {
        ...
        public UnpairedRow(String recipient, String sender, String amount, String uuid) {
            this.recipient = recipient; this.sender = sender; this.amount = amount; this.uuid = uuid;
        }
